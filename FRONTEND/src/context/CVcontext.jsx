...
export const CVProvider = ({ children }) => {
  ...
  const updateArraySection = (section, index, data) => {
    setFormData((prev) => {
      const currentArray = Array.isArray(prev[section])
        ? [...prev[section]]
        : [];
  ...
  const validatePersonalInfo = (data) => {
    const errors = {};
    const { firstName, lastName, email, professionalTitle, phone, location } =
      data || {};
  ...
  const fetchUserCVs = async () => {
    try {
      const response = await axios.get(`${API_URL}/cvbuilder/user`, {
